 ...
 * users row from the Clerk JWT. displayName comes from the identity claims
 * (required at signup — playbook identity rule); the arg is a fallback only.
 */
export const ensureUser = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing !== null) {
      ...
      if (email !== existing.email || timezone !== existing.timezone) {
        await ctx.db.patch(existing._id, { email, timezone });
      }
      return existing._id;
    }
