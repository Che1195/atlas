...
- Input is **computed, not hallucinated**: the period's applied revisions, new knowledge, confidence transitions (from revision snapshots), resolved/expired proposals, experiment activity, evidence adds — assembled by pure lib `reviewSections.ts`. The model writes only `sections.prose` (≤ 200 words) and `recurringThemes` grouping; ids come from the computed input.
